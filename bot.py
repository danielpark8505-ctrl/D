...
def owner_actions(call):
    ...
    if action == "op_back_main": send_owner_menu(call.message.chat.id, call.message.message_id)
        ...
        bot.send_message(call.message.chat.id, "🚀 **Testing API...**")
...
# --- AUTO POST (FIXED FOR 2026 HEADER + KEY ERROR) ---
def auto_post_task(manual=False, chat_id=None):
    ...
    headers = {
        ...
        "Authorization": f"Token token={token}" if token else ""
    }
    ...
    if not token:
        ...
        try:
            ...
            o = data['offers'][0]
            # Use .get() to prevent KeyError if 'price' or 'title' is missing
            title = o.get('title') or o.get('name') or "New Deal"
            price = o.get('price') or o.get('sale_price')
            link = o.get('url')
            img = o.get('image_url')
